...
def upload_to_post(instance, filename):
    new_file_name = (
        f"{slugify(instance.profile.user.first_name)}"
        f"-{uuid.uuid4()}{pathlib.Path(filename).suffix}"
    )
...
class Post(models.Model):
    ...
    image = models.ImageField(upload_to=upload_to_post, blank=True, null=True)
...
class Like(models.Model):
    profile = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="liked_posts"
    )
